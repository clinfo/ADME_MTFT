...
def evaluation_score(file_path, target, ground_truth_path):
    ...
    ground_truth = ground_truth_df["value"].str.strip('[]').astype(float)[:len(df)]
    predictions = df[f'{target}']
...
adme_paths = glob.glob('/data_st01/drug/itosho/ADMET/configs/accuracy_drug/adme/finetuning_learning_rate/test/*')
adme_paths.sort()
...
for i, adme_path in enumerate(adme_paths):
    r2_values, mae_values, rmse_values, pearson_values, length_list = [], [], [], [], []
    adme = adme_path.split('/')[-1]
    for folder in folders:
        file_path = os.path.join(adme_path, folder, 'predictions.csv')
...
with open('/data_st01/drug/itosho/ADMET/configs/accuracy_drug/adme/finetuning_learning_rate/evaluation.txt', 'w') as file:
...
df.to_csv('/data_st01/drug/itosho/ADMET/configs/accuracy_drug/adme/finetuning_learning_rate/evaluation.csv', index=False)
